...
// statsconsumer. If the retryable error doesn't provide a delay, exponential backoff is applied.
type Config struct {
	// Enabled indicates whether to not retry sending logs in case of receiving a retryable error from a downstream
	// statsconsumer. Default is false.
	...
	// a downstream statsconsumer. Once this value is reached, the data is discarded. It never stops if MaxElapsedTime == 0.
