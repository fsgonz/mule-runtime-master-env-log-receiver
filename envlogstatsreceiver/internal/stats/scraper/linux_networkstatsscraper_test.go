...
func TestLinuxNetworkStatsScraper(t *testing.T) {
	t.Run("Network stats parsed from the buffer with default interface", func(t *testing.T) {
	...
	t.Run("Network stats parsed from the buffer with second interface in the buffer", func(t *testing.T) {
		const wantedReceivedBytes = 3862937603
		const wantedTransmitBytes = 281882792
		assertExpectedNetUsageBytes("testdata/eth0_test.data", t, wantedReceivedBytes, wantedTransmitBytes, "eth0")
	})

	t.Run("Network stats parsed from the buffer with third interface in the buffer", func(t *testing.T) {
	...
	t.Run("Network stats parsed from the buffer with lo interface in the buffer", func(t *testing.T) {
...
func assertExpectedNetUsageBytes(testFile string, t *testing.T, wantedReceivedBytes uint64, wantedTransmitBytes uint64, interfaceName string) {
	...
	if err != nil {
		assert.Fail(t, "The following error occurred on retrieving the test buffer: "+err.Error())
	}
